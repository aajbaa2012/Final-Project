...
//       - PIC16F628a#2 is uesd as a storage unit to store and sent the
...
void main()
{
     ...
     while (1)
     {
           ...
           TMR1H = 0;
           TMR1L = 0;
           PORTA.F0 = 0;               //Disable exP2 signal
           PORTA.F1 = 0;               //Disable stP2 signal
           
           while(!PORTB.F1);           //Wait the echo (Read2 signal) to become 1
           ...
           while(PORTB.F5 != 0 && PORTB.F4 != 1);//D&C
           while(PORTB.F0 == 0);       //Wait for enP2 signal comming from Arduino
           
           PORTA.F1 = 1;               //Enable stP2 signal

           // Start sending data via UART
           UART1_Write_Text("GG2_");
           ...
           PORTA.F0 = 1;               //Enable the exP2 signal
